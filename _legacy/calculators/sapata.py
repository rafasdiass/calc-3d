...
class Sapata:
    """
    Classe responsável pelos cálculos de uma fundação do tipo Sapata.
    """

    def __init__(self, carga: float, fck: float, base: float, altura: float, capacidade_solo: float, peso_concreto: float = 25):
        """
        Inicializa uma instância da fundação Sapata.

        :param carga: Carga aplicada sobre a sapata (kN)
        :param fck: Resistência característica do concreto (MPa)
        :param base: Dimensão da base da sapata (m)
        :param altura: Altura da sapata (m)
    ...
    def calcular_area(self) -> float:
        """
        Calcula a área da base da sapata (m²).

        :return: Área da sapata (m²)
        """
        return self.base ** 2

    def calcular_volume_concreto(self) -> float:
        """
        Calcula o volume de concreto da sapata (m³).
    ...
    def calcular_peso_concreto(self) -> float:
        """
        Calcula o peso total do concreto da sapata (kN).
    ...
    def calcular_carga_admissivel(self) -> float:
        """
        Calcula a carga admissível da sapata com base na capacidade do solo.
    ...
    def calcular_armacao(self) -> Dict[str, float]:
        """
        Calcula a armadura necessária para a sapata.

        :return: Dicionário com a quantidade de aço e o diâmetro das barras
        """
        armadura_minima = 0.0015  # Proporção mínima de aço para sapata (0,15%)
        area_aco = armadura_minima * self.calcular_area() * self.altura  # Área de aço necessária (m²)

        # Supondo barras de 12 mm de diâmetro
    ...
    def gerar_relatorio(self) -> Dict[str, float]:
        """
        Gera um relatório completo com os cálculos da sapata.
